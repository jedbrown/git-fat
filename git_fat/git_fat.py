...
def difftreez_reader(input):
    ...
    while True:
        ...
        while True:
            ...
            buffer.append(head)
            if len(buffer) == 2:
                _, _, _, newhash, modflag = buffer[0].split()
                path = buffer[1]
...
class GitFat(object):
    ...
    def local_sha1_checksum(self, path: str):
        # return hashlib.md5(open(path, "rb").read()).hexdigest()
        # Cheat here
        return path

    def get_all_remote_files(self, bucket: str, prefix):
    ...
    def get_remote_item(self, file, remote_files):
        ...
        for remote_file in remote_files:
            # self.verbose(remote_file)
            if remote_file.get("Key", "") == file:
                return remote_file

        return output

    def get_aws_cmd(self, push, s3_bucket, s3_prefix, s3_extrapushargs, files):
        if not which("aws"):
        ...
        cmds = []
        remote_files = self.get_all_remote_files(s3_bucket, s3_prefix)
        # self.verbose(remote_files)
        if push:
            self.verbose("Pushing to %s/%s" % (s3_bucket, s3_prefix))
            for file in files:
            ...
            self.verbose("Pulling from %s/%s" % (s3_bucket, s3_prefix))
            # self.verbose(remote_files)
            for file in files:
                self.verbose("Processing {}".format(file))
                local_path = os.path.join(self.objdir, file)
    ...
    def get_rsync_command(self, push, files):
        ...
        s3_prefix = gitconfig_get("s3.prefix", file=cfgpath)
        s3_extrapushargs = gitconfig_get("s3.extrapushargs", file=cfgpath)
        if s3_bucket:
            return self.get_aws_cmd(push, s3_bucket, s3_prefix, s3_extrapushargs, files)
    ...
    def cmd_push(self, args):
        ...
        cmds = self.get_rsync_command(push=True, files=files)
        total = len(cmds)
        counter = 0
        for cmd in cmds:
            counter = counter + 1
            print(f"Executing file {counter} of {total}")
    ...
    def cmd_pull(self, args):
        ...
        if "--all" in args:
            refargs["all"] = True
        for arg in args:
            if arg.startswith("-") or len(arg) != 40 or os.path.exists(arg):
                continue
            rev = self.revparse(arg)
            if rev:
                refargs["rev"] = rev
        files = self.filter_objects(refargs, self.parse_pull_patterns(args))
        cmds = self.get_rsync_command(push=False, files=files)
        for cmd in cmds:
            self.verbose("Executing: %s" % " ".join(cmd))
            p = subprocess.Popen(cmd, stdin=subprocess.PIPE)
            p.communicate(input=b"\x00".join(tobytes(file) for file in files))
    ...
    def cmd_index_filter(self, args):
        ...
        if manage_gitattributes:
            ...
            stdout, stderr = hashobject.communicate(
                b"\n".join(
                    tobytes(l) for l in gitattributes_lines + gitattributes_extra
                )
                + b"\n"
            )
            updateindex.stdin.write(
